...
class Product_filter extends CI_Controller {
 ...
 function fetch_data()
 {
  //set here pagination and filter 
  sleep(1);
  ...
  // $config['total_rows'] = $this->product_filter_model->count_all($minimum_price, $maximum_price, $brand, $ram, $storage);
   $config['total_rows'] = $this->product_filter_model->count_all($minimum_price, $maximum_price);
  $config['per_page'] = 8;
  ...
  $output = array(
   ...
   'product_list'   => $this->product_filter_model->fetch_data($config["per_page"], $start, $minimum_price, $maximum_price)
  );
